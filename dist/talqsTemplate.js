...
var templateConfig = {};
templateConfig[components.Question] = [
      components.StemsWrapper,
      components.AnalyzeWrapper
    ];
templateConfig[components.StemsWrapper] = [
      components.Index,
      components.Source,
...
templateConfig[components.AnalyzeWrapper] = [
      components.Answer,
      components.Analyze,
      components.KnowledgePoint,
      components.QueID
    ];
templateConfig[components.ChildQSAnalyze] = [
      components.Answer,
      components.Analyze,
      components.KnowledgePoint,
      components.QueID
 ...
 */
var updateTemplateList = function (data) {
  ...
  var loop = function ( key ) {
    ...
    if (item.exclude && item.exclude.length && templateList.length) {
      templateList = templateList.filter(function (sub) { return item.exclude.indexOf(sub) === -1; });
    }
    // 添加新定义的组件或者覆盖内置组件
    ...
    // 模板重新赋值
    TalqsTemplate.config.templates[key] = templateList.filter(function (key) { return typeof key === 'string'; });
  };

  for (var key in data) loop( key );
};
